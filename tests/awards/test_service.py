        ...
        "awarders": [
            {
                ...
                "user_id": 50695,
                "awards": [
                    {"name": "Posthumous Blue", "description": "d", "image": "i",
                     "markers": ["bluepost", "1077284"]},
                    {"name": "Posthumous Red", "description": "d", "image": "i",
                     "markers": ["redpost"]},
                ],
            }
        ]
...
def world(session):
    ...
    session.add(
        Challenge(
            id=100, name="Test Challenge", description="",
    ...
    for image_id in (500, 501):
        session.add(
            Image(id=image_id, challenge_id=100, photographer_id=1, name=f"img{image_id}",
                  votes=[1] * 10, disqualified=False)
        )
    session.commit()
    return session
...
class TestSyncCatalog:
    def test_creates_awards(self, world):
        ...
        assert {a.slug for a in world.query(Award).all()} == {
            "posthumous-blue", "posthumous-red"
        }
...
class TestFindGrants:
    ...
    def test_only_the_awarders_own_comments_count(self, world):
        ...
        world.add(
            Comment(id=902, image_id=500, commenter_id=777,
                    raw_comment="<td>bluepost</td>", comment="x",
